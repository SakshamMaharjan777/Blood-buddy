...
## Session #29 (2026-09-17) — the runbook is walked in a browser: 32 screenshots, 35 assertions, all green ✅

Session #28 left the machine in the rehearsal's start state (pristine DB, Mailpit up and empty,
8081 free). This session used it: instead of *describing* the rehearsal, it was **run** — in a
browser, checkpoint by checkpoint — and photographed.

### 1. The tool (tracked, promoted from a local helper at the end of the session)
- **`tools/walkthrough/walkthrough.js`** (new, ~800 lines, puppeteer-core — the dependency
  `capture.js` already uses; the script resolves it from `tools/walkthrough/node_modules` after
  `npm install` **or** from the existing gitignored `.tools/shots/node_modules`, and it takes
  `BB_CHROME`/`BB_BASE`/`BB_MAILPIT`/`BB_WALK_OUT` overrides). It logs in
  through the **real login form** (`#login-role`/`#login-email`/`#login-pass`/`#login-submit`),
  then **clicks**: the register wizard, the search filters, the request form, the donor's Accept,
  the stock modal, the hospital's Mark fulfilled, the admin's suspend + forward.
- **`tools/walkthrough/gallery.js`** turns its report into **`.tools/wt/walkthrough/index.html`** — one
  page to page through (each shot with the checkpoint caption), plus `walkthrough.txt` for the
  machine-readable record.
- Every checkpoint **asserts the thing the runbook claims** and records PASS/FAIL, so the run is
  not a set of nice pictures: **35 assertions, 35 passing**.
- Overridable account (`BB_WALK_EMAIL=… node walkthrough.js 21`) so a single step can be re-run
  against an account an earlier pass created.

### 2. What the browser actually did (the checkpoints, with the numbers it printed)
| Runbook step | Driven in the browser | Result |
|---|---|---|
| 0:00 landing | 1440px, then a **360px** viewport | reflows; `scrollWidth 360` — no horizontal overflow |
| 0:00 emergency | `/CODE/HTML/emergency-request.html` with no session | renders for a guest |
| 1:00 register | wizard → bad email → then the real one | **inline error + banner, no `alert()` dialog**; `201`; **Mailpit 0 → 1** with *“Welcome to BloodBuddy — confirmation for Walk Through”* |
| 4:00 requester | login as `sita.g`, B+/Kathmandu, then O+/Bhaktapur | **6 donor cards**; widen fallback: *“no O+ profiles in Bhaktapur yet — showing O+ donors in all districts”* |
| 4:00 submit | 2 units B+ → TUTH, needed-by +2 days | `BB-9UG2U3` in the tracker; **11** ``Urgent … request in …`` mails in Mailpit |
| 7:00 donor | login as `arun.s`, board | 3 rows, **`BB-5MN8VX` (O−) absent**; Accept → **the row leaves his open list** |
| 9:30 hospital | login as `bloodbank@tuth`, inventory → modal → queue → fulfil | 8 groups, B+ **12 → 13**; queue = **its own 2 rows** (`BB-9UG2U3`, `BB-7HD4LN`), no foreign hospital; fulfil → B+ **11** (BR-4) |
| 11:30 admin | login as `saksham`, users table, suspend, moderation | 5 rows/page; suspend → the **row reads `Suspended`**; that account's next login → *“This account is suspended — please contact the platform administrator.”*; the unrouted guest request forwarded to Patan |
| 13:30 guardrails | wrong-role page, then logged out | admin → bounced to `admin-dashboard.html`; a **logged-out** visitor asking for `donor-dashboard.html` → `auth-login.html`; `/api/admin/users` → **403** (requester) and **401** (anonymous) |
| 14:30 tools | the three self-driving pages | **`REAL-BACKEND-DONE 42/42`**, **`QA-DONE 268/268`**, **`RESP-DONE 92/92`** |

### 3. The part that was not green first time — and why it is worth recording
A screenshot that is the *previous frame*, or that photographs the wrong part of the page, is
worse than no screenshot: it looks like evidence. Three things were wrong before they were right:
1. **Stale frames.** `14`/`15`, `16`/`17` and `20`/`21` came out **byte-identical** — a shot taken
   immediately after a DOM change can return the frame before it. The script now waits for a real
   compositor frame (double `requestAnimationFrame` + a beat) before every shot.
2. **Photographing nothing.** The admin shots showed the top of the dashboard (KPIs/charts) while
   the **users table is below the fold** — so “the row is Suspended” was photographed as the same
   KPI panel as the login shot. The script now scrolls the evidence into view, and re-applies the
   users-search after the suspend (the table re-renders from page 1, where the changed account is
   **not** — the first version proved nothing).
3. **Self-check added.** The run now asserts that **every shot is a distinct picture** and prints
   any byte-identical group in `walkthrough.txt`; the final pass reports *“all 30 shots are
   distinct”*. It also **waits for the visible change** it claims (the accepted row leaving the
   board; the fulfilled row leaving the queue) instead of trusting a fixed sleep.
Two bugs in the script itself surfaced on the way: `page.click(sel, {clickCount: 3})` **did not
clear** the register email field, so the first pass registered the syntactically-valid
``not-an-emailwalk-…@example.com`` (the app was right to accept it — the *script* was wrong), and
step 15 waited on a selector that could never appear.
**One runbook correction came out of it:** §1 1:00 said “inline field error, **no alert box**”.
The page shows the inline error **and** a one-line in-page summary banner; what is absent is a
native `alert()` dialog. The wording now says exactly that (and records the 0 → 1 mail).

### 4. The demo state was put back again
After the run: **21 users / 14 donors / 5 hospitals / 40 inventory / 4 requests / 3 notifications /
7 timeline / 39 affiliations / 0 declines / 0 donor photos**, TUTH B+ = **12**, `BB-5MN8VX`
**PENDING + unrouted** (`E28094`), only **Bikash Tamang** suspended. Mailpit emptied (it had ended
with the walkthrough's ~30 mails).

### State at session end
- **Artifacts (all gitignored, local):** `.tools/wt/walkthrough/` — **32 PNGs**, `index.html`
  (the gallery), `walkthrough.txt` (steps + assertions + shots + the distinctness self-check);
  the tool itself is tracked: `tools/walkthrough/{walkthrough.js,gallery.js,package.json,README.md}`.
- **DB pristine** (counts above), **Mailpit up on 8025 with an empty inbox**, app **stopped and
  8081 free** for IntelliJ's ▶. A short verification run of the promoted tool (`--first 3`, output
  redirected with `BB_WALK_OUT`, steps 01–03 write nothing) confirmed it runs from its new home
  and left the pristine counts untouched.
- **Tracked changes:** `PROGRESS.md` (this section), `docs/DEMO_RUNBOOK.md` (the 1:00 validation
  wording, the browser-walked note at 1:00, and a new row in §0's *every number* table) and
  `.gitignore` (ignore `tools/walkthrough/node_modules/`), plus the new `tools/walkthrough/` tool.

### Hand-off — next session starts here
1. **The operator's own rehearsal is still the last item, and it is a human's job:** drive
   `docs/DEMO_RUNBOOK.md` §0 → §1 → §6 by hand, with IntelliJ's ▶, Mailpit up and the DB pristine
   (all three are already true). The scripted walkthrough above is the safety net, not a
   substitute — the points worth *looking* at are the hospital queue for a TUTH session and one
   mail per registration, and both are in the gallery with their numbers.
2. ~~Promote the walkthrough tool into tracked `tools/`~~ — **done**: `tools/walkthrough/` ships the
   script, a gallery builder, `package.json` and a README (prerequisites, the step numbering, the
   `BB_*` overrides, and the warning that it **writes to the demo database**), and the runbook's
   §0 table now carries the row that reproduces it.
3. Optional polish: the diagrams are dense at 6 pt, and the runbook's §0 table could print the two
   new check counts side by side.
...
- **Evidence from this session** (all gitignored under `.tools/`): `wt/db-before-session28.sql`
  ...
  `wt/mailpit-s28b.log`.
- **Committed and pushed as `f654841`** — the combined #26 + #27 set (the backend BR-5 fix,
  ...
  `build_montages.py`, `assets/shots/` — 24 PNGs, 14 MB — and `fig1_1/fig3_2/fig3_5/fig4_1`):
  **44 files, +1344/−185, `origin/main` moved `1c4ca14..f654841`** (the session-#27 hand-off’s
  item 5, done in this session). This session’s own diff is `PROGRESS.md` alone,
  **+433/−17**, and it is committed on top of `f654841`.
...
2. ~~Commit and push the combined #26 + #27 set~~ — **done, `f654841` on `origin/main`** (this
   session adds only `PROGRESS.md`).
